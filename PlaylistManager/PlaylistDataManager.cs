...
public class PlaylistDataManager : IInitializable, IDisposable
{
	...
	internal PlaylistDataManager(AnnotatedBeatmapLevelCollectionsViewController annotatedBeatmapLevelCollectionsViewController, LevelPackDetailViewController levelPackDetailViewController, LevelFilteringNavigationController levelFilteringNavigationController, [InjectOptional] FoldersViewController foldersViewController, List<ILevelCollectionUpdater> levelCollectionUpdaters, List<ILevelCollectionsTableUpdater> levelCollectionsTableUpdaters, List<IBeatmapLevelUpdater> beatmapLevelUpdaters, List<IParentManagerUpdater> parentManagerUpdaters)
	{
		...
		emptyBeatmapLevelPack = new BeatmapLevelPack("custom_levelpack_CustomLevels", "Custom Levels", "Custom Levels", BeatSaberMarkupLanguage.Utilities.ImageResources.BlankSprite, BeatSaberMarkupLanguage.Utilities.ImageResources.BlankSprite, PackBuyOption.Default, Array.Empty<BeatmapLevel>(), PlayerSensitivityFlag.Safe);
	}
	...
	private void LevelCollectionTableView_DidSelectLevelEvent(BeatmapLevel beatmapLevel)
	{
		...
		foreach (IBeatmapLevelUpdater beatmapLevelUpdater in beatmapLevelUpdaters)
		{
	...
	private void LevelCollectionsTableUpdater_LevelCollectionTableViewUpdated(IReadOnlyList<BeatmapLevelPack> annotatedBeatmapLevelCollections, int indexToSelect)
	{
		...
		{
			annotatedBeatmapLevelCollectionsViewController.SetData(annotatedBeatmapLevelCollections, indexToSelect, hideIfOneOrNoPacks: false);
			levelFilteringNavigationController.InvokeMethod("HandleAnnotatedBeatmapLevelCollectionsViewControllerDidSelectAnnotatedBeatmapLevelCollection", annotatedBeatmapLevelCollections[indexToSelect]);
			return;
		}
		annotatedBeatmapLevelCollections = new BeatmapLevelPack[1] { emptyBeatmapLevelPack };
		annotatedBeatmapLevelCollectionsViewController.SetData(annotatedBeatmapLevelCollections, 0, hideIfOneOrNoPacks: true);
		levelFilteringNavigationController.InvokeMethod("HandleAnnotatedBeatmapLevelCollectionsViewControllerDidSelectAnnotatedBeatmapLevelCollection", annotatedBeatmapLevelCollections[0]);
	}
}
